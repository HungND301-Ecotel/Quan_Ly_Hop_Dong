 ...
 */
const getContractBadge = (status: string, subStatus?: string, signingFlows?: Contract['signingFlows']) => {
  ...
  if (status === 'Archive' || status === 'Archived') {
    return parentStatus ? (
      <Badge className={`${parentStatus.background} ${parentStatus.foreground} ${badgeClass} w-full`}>
        {parentStatus.title}
      </Badge>
    ) : <Badge variant='outline' className={badgeClass}>Không xác định</Badge>;
  }
  ...
  return (
    <div className='flex flex-col items-end gap-1 w-full'>
      ...
      {childStatus && (
        <Badge className={`${childStatus.background} ${childStatus.foreground} ${badgeClass} w-full`}>
          {childStatus.title}
          {awaitingDeptName && ` - ${awaitingDeptName}`} {/* ✅ Thêm tên phòng ban */}
        </Badge>
      )}
      {!parentStatus && !childStatus && (
        <Badge variant='outline' className={`${badgeClass} w-full`}>Không xác định</Badge>
      )}
    </div>
  );
};
...
const isPrinciple = (c: Contract) => c.contractFormat === 0 || c.contractFormat === 1;
...
const MOCK_DEBT_DATA = [
  { name: 'Tổng công nợ', buyLabel: '', sellLabel: '', buy: 1_200_000_000, sell: 980_000_000 },
  { name: '', buyLabel: 'Đã trả', sellLabel: 'Đã thu', buy: 800_000_000, sell: 650_000_000 },
  { name: '', buyLabel: 'Chưa trả', sellLabel: 'Chưa thu', buy: 400_000_000, sell: 330_000_000 },
  { name: 'Quá hạn', buyLabel: '', sellLabel: '', buy: 150_000_000, sell: 80_000_000 },
];
...
const Dashboard: React.FC = () => {
  ...
  // ── Filter state ──
  const [contractListFilter, setContractListFilter] = useState<FilterOption>('ALL');
  const [selectedModel, setSelectedModel] = useState<string>('ALL');
  const [recentContractFilter, setRecentContractFilter] = useState<string>('ALL');
  const [expiringContractFilter, setExpiringContractFilter] = useState<string>('ALL');

  // ── Data state ──
  const [allContracts, setAllContracts] = useState<Contract[]>([]);
  const [pendingContracts, setPendingContracts] = useState<Contract[]>([]);
  const [soonExpiredContracts, setSoonExpiredContracts] = useState<Contract[]>([]);
  const [paymentDueSoonContracts, setPaymentDueSoonContracts] = useState<Contract[]>([]);
  ...
  // ── Reload keys ──
  const [reloadKeys, setReloadKeys] = useState({ contracts: 0, recent: 0, expiring: 0 });
  ...
  useEffect(() => { fetchAllContracts(); }, [fetchAllContracts, reloadKeys.contracts, reloadKeys.recent]);
  useEffect(() => { fetchPendingContracts(); }, [fetchPendingContracts, reloadKeys.contracts]);
  useEffect(() => { fetchSoonExpired(); }, [fetchSoonExpired, reloadKeys.expiring]);
  useEffect(() => { fetchPaymentDueSoon(); }, [fetchPaymentDueSoon, reloadKeys.contracts]);

  // ─── Derived: Pie chart ─────────────────────────────────────────────────────

  const filteredContractsForChart = useMemo(() => {
    switch (contractListFilter) {
      case 'PRINCIPLE': return allContracts.filter(isPrinciple);
      case 'ECONOMIC_BUY': return allContracts.filter(isEconomicBuy);
      case 'ECONOMIC_SELL': return allContracts.filter(isEconomicSell);
  ...
  const recentContracts = useMemo(() => {
    ...
    if (recentContractFilter !== 'ALL') {
      list = list.filter(c => c.contractFormat === parseInt(recentContractFilter));
    }
    return list
      .sort((a, b) => new Date(b.draftDate ?? '').getTime() - new Date(a.draftDate ?? '').getTime())
      .slice(0, 5);
  ...
  const expiringContracts = useMemo(() => {
    ...
    if (expiringContractFilter !== 'ALL') {
      list = list.filter(c => c.contractFormat === parseInt(expiringContractFilter));
    }
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4'>
          ...
          trigger={
            <Card className='hover:shadow-lg cursor-pointer transition-all border-l-4 border-l-blue-500 bg-blue-50 hover:bg-blue-100'>
              <CardContent className='flex items-center gap-4 p-4'>
                <div className='p-3 bg-blue-500 rounded-lg'><FilePlus className='h-6 w-6 text-white' /></div>
                <div className='flex-1'>
                  <p className='text-sm text-blue-600 font-semibold mb-1'>Tạo mới hợp đồng</p>
                </div>
              </CardContent>
            </Card>
          }
        ...
        <Link to='/contract/approval/pending'>
          <Card className='hover:shadow-lg cursor-pointer transition-all border-l-4 border-l-green-500 bg-green-50 hover:bg-green-100'>
            <CardContent className='flex items-center gap-4 p-4'>
              <div className='p-3 bg-green-500 rounded-lg'><FileCheck className='h-6 w-6 text-white' /></div>
              <div className='flex-1'>
                <p className='text-sm text-green-600 font-semibold mb-1'>Phê duyệt hợp đồng</p>
                <p className='text-xs text-green-700 mt-1'>
                  {loadingPending ? 'Đang tải...' : `(${pendingContracts.length}) Hợp đồng chờ duyệt`}
                </p>
              </div>
            </CardContent>
          </Card>
        ...
        <Link to='/contract/expired'>
          <Card className='hover:shadow-lg cursor-pointer transition-all border-l-4 border-l-orange-500 bg-orange-50 hover:bg-orange-100'>
            <CardContent className='flex items-center gap-4 p-4'>
              ...
              <div className='flex-1'>
                <p className='text-sm text-orange-600 font-semibold mb-1'>Hợp đồng sắp hết hạn</p>
                <p className='text-xs text-orange-700 mt-1'>
                  {loadingSoonExpired ? 'Đang tải...' : `(${soonExpiredContracts.length}) Hợp đồng trong 10 ngày tới`}
                </p>
              </div>
            </CardContent>
          </Card>
        ...
        <Link to='/contract/payment-due'>
          <Card className='hover:shadow-lg cursor-pointer transition-all border-l-4 border-l-purple-500 bg-purple-50 hover:bg-purple-100'>
            <CardContent className='flex items-center gap-4 p-4'>
              <div className='p-3 bg-purple-500 rounded-lg'><CreditCard className='h-6 w-6 text-white' /></div>
              <div className='flex-1'>
                <p className='text-sm font-semibold text-purple-900'>Hợp đồng đến hạn thanh toán</p>
                <span className='text-xs text-purple-700 mt-1'>
                  {loadingPaymentDue ? 'Đang tải...' : `(${paymentDueSoonContracts.length}) Hợp đồng trong 7 ngày tới`}
                </span>
              </div>
            </CardContent>
          </Card>
            ...
            <CardTitle className='flex items-center justify-between'>
              <div className='flex flex-col gap-1'>
                <div className='flex items-center gap-2'>
                  ...
                  <span className='text-lg font-semibold'>Quản lý hợp đồng</span>
                </div>
                <span className='text-sm text-muted-foreground'>
                  {loadingContracts ? 'Đang tải...' : `Tổng số: ${filteredContractsForChart.length} hợp đồng`}
                </span>
              </div>

              <div className='flex items-center gap-3'>
                <label className='text-sm font-medium whitespace-nowrap'>Lọc:</label>
                <Select value={contractListFilter} onValueChange={val => setContractListFilter(val as FilterOption)}>
                  <SelectTrigger className='w-56 h-9 px-3 rounded-md'>
                    <SelectValue placeholder='Chọn loại hợp đồng' />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value='ALL'>Tất cả mẫu hợp đồng</SelectItem>
                    <SelectItem value='PRINCIPLE'>Hợp đồng nguyên tắc</SelectItem>
                    <SelectItem value='ECONOMIC_BUY'>Hợp đồng kinh tế - Mua</SelectItem>
                    <SelectItem value='ECONOMIC_SELL'>Hợp đồng kinh tế - Bán</SelectItem>
                  </SelectContent>
                </Select>
                <button onClick={() => handleReload('contracts')} className='p-1.5 rounded-md hover:bg-muted transition-colors' title='Tải lại'>
                  ...
                  <Pie
                    ...
                    label={entryProps => {
                      ...
                      return renderCustomizedLabel({
                        ...rest,
                        name: payload.name, color: payload.color, percent: entryProps.percent,
                      } as PieLabelProps);
                    }}
        ...
        <Card className='flex flex-col h-full'>
            ...
            <div className='flex items-center justify-between'>
              ...
              <div className='flex items-center gap-2'>
                <Select value={selectedModel} onValueChange={setSelectedModel}>
                  <SelectTrigger className='w-50'><SelectValue placeholder='Chọn loại hợp đồng' /></SelectTrigger>
                  <SelectContent>
          ...
          <CardContent className='flex-1 flex flex-col gap-4'>
            <div className='flex gap-2 items-center justify-center flex-wrap'>
              ...
              <Select value={startMonth?.toString()} onValueChange={val => form.setValue('startMonth', parseInt(val))}>
                <SelectTrigger className='w-28'><SelectValue placeholder='Tháng' /></SelectTrigger>
                <SelectContent>
              ...
              <Select value={startYear?.toString()} onValueChange={val => form.setValue('startYear', parseInt(val))}>
                <SelectTrigger className='w-24'><SelectValue placeholder='Năm' /></SelectTrigger>
                <SelectContent>
              ...
              <Select value={endMonth?.toString()} onValueChange={val => form.setValue('endMonth', parseInt(val))}>
                <SelectTrigger className='w-28'><SelectValue placeholder='Tháng' /></SelectTrigger>
                <SelectContent>
                  {Array.from({ length: 12 }, (_, i) => i + 1).map(m => (
                    <SelectItem key={m} value={m.toString()}>Tháng {m}</SelectItem>
              ...
              <Select value={endYear?.toString()} onValueChange={val => form.setValue('endYear', parseInt(val))}>
                <SelectTrigger className='w-24'><SelectValue placeholder='Năm' /></SelectTrigger>
                <SelectContent>
                  {Array.from({ length: 11 }, (_, i) => 2020 + i).map(y => (
                    <SelectItem key={y} value={y.toString()}>{y}</SelectItem>
            ...
            <div className='flex-1 min-h-75'>
              <ResponsiveContainer width='100%' height='100%'>
                <BarChart data={MOCK_DEBT_DATA}>
                  ...
                  <XAxis dataKey='name' tick={{ fontSize: 12 }} height={80} textAnchor='middle' interval={0} />
                  <YAxis
                    tick={{ fontSize: 12 }}
                    tickFormatter={v => {
                  ...
                  />
                  <Tooltip
                    formatter={(value: number, name: string) => [`${value.toLocaleString('vi-VN')} đ`, name]}
                    contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}
                  />
                  <Legend verticalAlign='top' height={36} />
                  <Bar dataKey='buy' name='Hợp đồng kinh tế mua' fill='#3B82F6' radius={[4, 4, 0, 0]}>
                    <LabelList dataKey='buy' position='top'
                      formatter={(v: number) => {
                        if (v === 0) return '';
                        if (v >= 1e9) return `${(v / 1e9).toFixed(1)}B`;
                    ...
                    />
                    <LabelList dataKey='buyLabel' position='bottom' offset={10}
                      style={{ fontSize: 11, fontWeight: 500, fill: '#3B82F6' }}
                    />
                  </Bar>
                  <Bar dataKey='sell' name='Hợp đồng kinh tế bán' fill='#F97316' radius={[4, 4, 0, 0]}>
                    <LabelList dataKey='sell' position='top'
                      formatter={(v: number) => {
                        if (v === 0) return '';
                        if (v >= 1e9) return `${(v / 1e9).toFixed(1)}B`;
                        if (v >= 1e6) return `${(v / 1e6).toFixed(1)}M`;
                    ...
                    />
                    <LabelList dataKey='sellLabel' position='bottom' offset={10}
          ...
          <CardHeader>
            <div className='flex items-center justify-between'>
              ...
              <div className='flex gap-2'>
                <Select value={recentContractFilter} onValueChange={setRecentContractFilter}>
                  <SelectTrigger className='w-fit h-8 text-xs'><SelectValue placeholder='Mẫu Hợp đồng' /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value='ALL'>Tất cả mẫu hợp đồng</SelectItem>
                ...
                <button onClick={() => handleReload('recent')} className='p-1.5 rounded-md hover:bg-muted transition-colors' title='Tải lại'>
                  <RefreshCw className='h-4 w-4 text-muted-foreground' />
                </button>
              </div>
            </div>
          ...
          <CardContent>
            <div className='space-y-4'>
              {loadingContracts ? (
                ...
                <p className='text-center text-sm text-muted-foreground py-4'>Không có dữ liệu</p>
                ...
                <Link key={c.id} to={`/contracts/${c.id}`} className='block'>
                  <div className='grid grid-cols-12 gap-2 p-3 rounded-lg border border-border hover:bg-accent transition-colors items-center'>
                    <div className='col-span-4 flex flex-col min-w-0'>
                      <p className='font-medium truncate text-sm'>{c.contractNumber}</p>
                      <p className='text-xs text-muted-foreground truncate mt-0.5'>{c.title}</p>
                    </div>
                    <div className='col-span-5'>
                      <p className='text-sm font-medium text-right truncate'>{c.partnerName}</p>
                    </div>
                    <div className='col-span-3 flex justify-end'>
                      {getContractBadge(c.status ?? '', c.subStatus, c.signingFlows)}
                    </div>
                  </div>
                </Link>
              ))}
            ...
            <Link to='/contract/all'>
              <Button variant='outline' className='w-full mt-4'>Xem thêm</Button>
            </Link>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <div className='flex items-center justify-between'>
              ...
              <div className='flex gap-2'>
                <Select value={expiringContractFilter} onValueChange={setExpiringContractFilter}>
                  <SelectTrigger className='w-fit h-8 text-xs'><SelectValue placeholder='Mẫu Hợp đồng' /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value='ALL'>Tất cả mẫu hợp đồng</SelectItem>
                    {Object.values(ContractFormat).map(t => (
                ...
                <button onClick={() => handleReload('expiring')} className='p-1.5 rounded-md hover:bg-muted transition-colors' title='Tải lại'>
                  <RefreshCw className='h-4 w-4 text-muted-foreground' />
                </button>
              </div>
            </div>
                ...
                <Link key={c.id} to={`/contracts/${c.id}`}>
                  <div className='grid grid-cols-12 gap-2 p-3 rounded-lg border border-warning/20 bg-warning/5 hover:bg-warning/10 transition-colors items-center'>
                    <div className='col-span-5 flex flex-col min-w-0'>
                      <p className='font-medium text-sm truncate'>{c.contractNumber}</p>
                      <p className='text-xs text-muted-foreground truncate mt-0.5'>{c.title}</p>
                    </div>
                    <div className='col-span-4'>
                      <p className='text-sm font-medium truncate text-right'>{c.partnerName}</p>
                    </div>
