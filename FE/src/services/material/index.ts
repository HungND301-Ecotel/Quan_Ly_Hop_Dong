...
async function getMaterialList(params: {
  ...
  keyword?: string;
  isSynced?: boolean;
}): Promise<{ data: Material[]; totalCount: number; totalPages: number }>;
...
async function getOtherMaterialList(params: {
  pageNumber: number;
  pageSize: number;
  keyword?: string;
}): Promise<{ data: Material[]; totalCount: number; totalPages: number }>;
...
async function createMaterial(body: CreateMaterialReq) {
  return await api.post<boolean, CreateMaterialReq>(API.MATERIALS.CREATE, body);
}
...
async function syncMaterial(body: { sourceConnectionId: string }) {
  return await api.post<string, { sourceConnectionId: string }>(
    '/material/sync',
    body
  );
}
