...
public record CreateContractStructureCatalogCommand(string Name, string? Code, string? Description, bool IsActive = true) : IRequest<Guid>;
    ...
    : IRequestHandler<CreateContractStructureCatalogCommand, Guid>
{
    public async Task<Guid> Handle(CreateContractStructureCatalogCommand request, CancellationToken cancellationToken)
        => await service.CreateAsync(request.Name, request.Code, request.Description, request.IsActive);
}
