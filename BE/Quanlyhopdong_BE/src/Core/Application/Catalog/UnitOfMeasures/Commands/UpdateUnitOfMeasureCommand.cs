...
public record UpdateUnitOfMeasureCommand(Guid Id, string Code, string Name, bool IsActive, string? Note) : IRequest<bool>;
    ...
    : IRequestHandler<UpdateUnitOfMeasureCommand, bool>
{
    ...
    {
        return await service.UpdateAsync(request.Id, request.Code, request.Name, request.IsActive, request.Note);
    }
}
