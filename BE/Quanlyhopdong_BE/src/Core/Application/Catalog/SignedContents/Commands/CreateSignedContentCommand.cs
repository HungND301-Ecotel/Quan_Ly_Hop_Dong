...
public record CreateSignedContentCommand(string Title, Guid Level3CodeId) : IRequest<Guid>;
...
public class CreateSignedContentCommandHandler(ISignedContentService signedContentService) : IRequestHandler<CreateSignedContentCommand, Guid>
{
    ...
    {
        return await signedContentService.CreateAsync(request.Title, request.Level3CodeId);
    }
}
