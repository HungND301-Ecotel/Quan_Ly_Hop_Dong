...
public record UpdateSignedContentCommand(Guid Id, string Title, Guid Level3CodeId, string? Description) : IRequest<bool>;
...
public class UpdateSignedContentCommandHandler(ISignedContentService signedContentService) : IRequestHandler<UpdateSignedContentCommand, bool>
{
    ...
    {
        return await signedContentService.UpdateAsync(request.Id, request.Title, request.Level3CodeId, request.Description);
    }
}
